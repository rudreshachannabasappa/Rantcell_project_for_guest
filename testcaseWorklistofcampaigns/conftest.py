...
def pytest_sessionstart(session):
    if active_threads == 0:
        try:
            if not os.path.exists(config.test_data_folder_rootpath):
                e = Exception
                raise e
            if os.path.exists(config.test_data_folder_rootpath):
                try:
                    create_folder_for_rantcell_data_and_ExcelReport(config.test_data_folder_rootpath, config.source_dest)
                except Exception:
                    print(str(Exception))
                finally:
                    testRun_downloadfile_path(config.test_run_download_file_path)
                    time.sleep(2)
                    excel_report_path_(config.test_run_excelreportdata_path)
                    f1 = open(config.test_run_download_file_path, "r")
                    testrundownloadfolder = f1.read()
                    f1.close()
                    test_case_downloading_files_path_timestamps = config.test_case_downloading_files_path_timestamp + testrundownloadfolder
                    ...
                    f1.close()
                    test_run_excel_report_pathtimestamp = config.excel_report_path + testrunexcelfolder
                    create_folder_for_excelreport(destination_folder=test_run_excel_report_pathtimestamp)
                    Updating_source_folder(config.updating_source_folders, config.test_data_folder_rootpath)
        except Exception as e:
            with allure.step(f"Goto the directory {config.test_data_folder_rootpath} and update the file as per your requirement, then re-run"):
                if not os.path.exists(config.test_data_folder_rootpath):
                    create_folder_for_rantcell_data_and_ExcelReport(config.test_data_folder_rootpath, config.source_dest)
                    Updating_source_folder(config.updating_source_folders, config.test_data_folder_rootpath)
                    pytest.fail(f"Goto the directory {config.test_data_folder_rootpath} and update the file as per your requirement")
...
def setup(request):
    ...
    test_data_folder_rootpath = config.test_case_downloading_files_path_timestamp + testrundownloadfolder
    ...
    # Create a unique folder path for each test case
    # Create a sanitized test case name for the folder
            ...
            if os.path.exists(config.test_data_folder_rootpath):
                with allure.step("Data file is present"):
                    with allure.step(f"Item count: {item_count}"):
                        ...
                        high_level_excel_report_path = config.excel_report_path + testrunexcelfolder
